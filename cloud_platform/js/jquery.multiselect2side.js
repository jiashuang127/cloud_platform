...
(function($)
{
	jQuery.fn.multiselect2side = function (o) {
		...
		return this.each(function () {
				...
				"<div class='ms2side__div'>" +
					...
					"<div class='ms2side__options'>" +
						...
						(
                            ...
                            "<p class='AddOne' title='Add Selected'><span><i class='iconfont icon-youjiantou'></i></span></p>" +

						"<p class='RemoveOne' title='Remove Selected'><span><i class='iconfont icon-zuojiantou'></i></span></p>" /*+
						"<p class='RemoveAll' title='Remove All'>&laquo;</p>"*/)
						:
						(/*"<p class='AddAll' title='Add All'>&laquo;</p>" +*/
                            "<p class='AddOne' title='Add Selected'><span><i><i class='iconfont icon-youjiantou'></i></span></p>" +

						"<p class='RemoveOne' title='Remove Selected'><span><i><i class='iconfont icon-zuojiantou'></i></i></span></p>" /*+
			...
			var $opts = el.find("option"),$ele2r = $opts.filter("option:disabled,option:selected");
			$ele2r.clone().appendTo(rightSel);
			$opts.not($ele2r).clone().appendTo(leftSel);
		...
		/*	// SELECT FIRST LEFT ITEM
			...
			rightSel.dblclick(function () {
				...
				// if($opts.length == o.minSelected) return;
				// if($opts.filter("option:selected").length == $opts.length ) return;
				$(this).find("option:selected").not("option:disabled").each(function(i, selected){
			...
			$(this).next().find('.ms2side__options').children().click(function () {
				if (!$(this).hasClass("ms2side__hide")) {
					...
					else if ($(this).hasClass("RemoveOne") && rightSel.find("option").length > o.minSelected ) {
						var $opts = rightSel.find("option");
						// if($opts.filter("option:selected").length == $opts.length ) return;
						$opts.filter("option:selected").not("option:disabled").each(function(i, selected){
							$(this).remove().appendTo(leftSel);
							el.find("[value=" + $(selected).val() + "]").prop("selected", false).remove().appendTo(el);
						});
					}
